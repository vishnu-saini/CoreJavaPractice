...
function menu_collapse (n_id) {
	...
	if(select1!=null )
		if(select1.style.visibility=="hidden" && n_id!=10)
		{
	...
	if(select2!=null )
		if(select2.style.visibility=="hidden" && n_id!=10)
		{
	...
	if(select3!=null )
		if(select3.style.visibility=="hidden" && n_id!=10)
		{
	...
	if(select4!=null )
		if(select4.style.visibility=="hidden" && n_id!=10)
		{
...
function menu_onclick (n_id) {
	...
	//alert(Boolean(this.a_index[n_id].a_config[1]));
...
function menu_item (o_parent, n_order) {
	...
	var n_width = this.getprop('width');
	var n_left = this.getprop('left');
	
	// recalculate x coordinate, if item size is mentioned in config
	if (this.a_config[2] && this.a_config[2]['wd'])
		n_width = this.a_config[2]['wd'];
	
	if (n_order && o_parent.a_children[n_order - 1].a_config[2] && o_parent.a_children[n_order - 1].a_config[2]['wd']) 
		n_left = o_parent.a_children[n_order - 1].a_config[2]['wd'] - 1;
		
	this.n_x = n_order
		? o_parent.a_children[n_order - 1].n_x + n_left
